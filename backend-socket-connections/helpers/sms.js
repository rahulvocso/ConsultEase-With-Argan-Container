...
const generateOTP = async (user) => {
  ...
  console.log("sendSMS Response OTP", otp);

  user.otp = otp;
  user.save();

  /* changes made on 20dec 3:09 after sir sent sms.js on flock */
  msg91.sendSMS(args, function (err, response) {
    if (err) throw new Error(err);

    if (response && response.type == "error") throw new Error(response.message);
